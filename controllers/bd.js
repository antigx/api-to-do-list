...
export const insertTodo = (id, title, text, color, favorite) => {
...
export const getDBTodos = (callback) => {
  db.all("SELECT * FROM todo", (err, rows) => {
    ...
    const jsonData = JSON.stringify(rows); // Convert rows to JSON

    callback(rows); // Pass JSON data to the callback
  });
};

export const updateTodo = (id, updatedProperties) => {
